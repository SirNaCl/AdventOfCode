...
def parse(puzzle_input):
    """Parse input."""
    tow, des = puzzle_input.split("\n\n")
    t = re.findall(r"\w+", tow)
    d = des.split("\n")
    return t, d
...
def is_possible(design: str):
    ...
    for t in TOWELS:
        if design.startswith(t):
            solutions += is_possible(design[len(t):])
...
def part1(data):
    ...
    return sum([1 if is_possible(dd) > 0 else 0 for dd in d])
